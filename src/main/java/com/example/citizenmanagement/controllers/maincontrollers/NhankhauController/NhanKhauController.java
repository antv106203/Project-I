...
public class NhanKhauController implements Initializable {
    ...
    public ChoiceBox luachontimkiemnhankhau;

    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {

        themmoi_btn.setOnAction(actionEvent -> onThemmoi());
        khoitaochoiceboxNhanKhau();
        ...
        tim_kiem_text.textProperty().addListener((observableValue, oldval, newval) -> {
            if(newval.isEmpty()){
                ...
                if(luachontimkiemnhankhau.getValue() == "Mã Nhân Khẩu") {
                    ResultSet resultSet = Model.getInstance().getDatabaseConnection().nhanKhau_timkiemTheoMaNhanKhau(tim_kiem_text.getText());
                    list_view.getItems().clear();
                    try {
                        if (resultSet.isBeforeFirst()) {
                            while (resultSet.next()) {
                                String ma = resultSet.getString(1);
                                boolean chuaChet = Model.getInstance().getDatabaseConnection().checkKhaiTu(ma);
                ...
                else if(luachontimkiemnhankhau.getValue() == "Số CCCD"){
                    ResultSet resultSet = Model.getInstance().getDatabaseConnection().nhanKhau_timkiemTheoSoCCCD(tim_kiem_text.getText());
                    list_view.getItems().clear();
                    try {
                        if (resultSet.isBeforeFirst()) {
                            while (resultSet.next()) {
                ...
                else if(luachontimkiemnhankhau.getValue() == "Họ Và Tên"){
                    ResultSet resultSet = Model.getInstance().getDatabaseConnection().nhanKhau_timkiemTheoHoVaTen(tim_kiem_text.getText());
                    list_view.getItems().clear();
                    try {
                        if (resultSet.isBeforeFirst()) {
                            while (resultSet.next()) {
    ...
    public void khoitaochoiceboxNhanKhau(){
        luachontimkiemnhankhau.getItems().addAll("Mã Nhân Khẩu","Số CCCD", "Họ Và Tên");
        luachontimkiemnhankhau.setValue("Mã Nhân Khẩu");
    }
}
