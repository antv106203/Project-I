...
public class ThemChuHoKhauController implements Initializable {
    ...
    public ChoiceBox luachonthemmoihokhau;
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        search_text.textProperty().addListener((observableValue, oldval, newval) -> {
            if(newval.isEmpty()){
                ...
                if(luachonthemmoihokhau.getValue() == "Mã Nhân Khẩu") {
                    ResultSet resultSet1 = Model.getInstance().getDatabaseConnection().nhanKhau_timkiem_chua_co_nha_theo_manhankhau( search_text.getText());
                ...
                else if(luachonthemmoihokhau.getValue() == "Số CCCD"){
                    ResultSet resultSet1 = Model.getInstance().getDatabaseConnection().nhanKhau_timkiem_chua_co_nha_theo_soCCCD( search_text.getText());
                    listView.getItems().clear();
                    try {
                        if (resultSet1.isBeforeFirst()) {
                            while (resultSet1.next()) {
                ...
                else if(luachonthemmoihokhau.getValue() == "Họ Và Tên"){
                    ResultSet resultSet1 = Model.getInstance().getDatabaseConnection().nhanKhau_timkiem_chua_co_nha_theo_HoTen( search_text.getText());
                    listView.getItems().clear();
                    try {
                        if (resultSet1.isBeforeFirst()) {
                            while (resultSet1.next()) {
    ...
    public void khoiTaoLuaChonTimKiem(){
        luachonthemmoihokhau.getItems().addAll("Mã Nhân Khẩu","Số CCCD", "Họ Và Tên");
        luachonthemmoihokhau.setValue("Mã Nhân Khẩu");
    }
}
